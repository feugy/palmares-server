...
const optsSchema = Joi.object({
  ...
  suffix: Joi.string().optional().default('').description('collection suffix, usefull for parallel testing')
...
module.exports = class MongoDB extends Storage {
  ...
  /** @inheritdoc */
  async find (modelClass, criteria, filtered = null) {
    const collection = await this._getCollection(modelClass)
    const cursor = await collection.find(criteria).project(makeFilter(filtered)).toArray()
